---
name: proof
description: Run human-in-the-loop review loops over markdown via Proof (proofeditor.ai) — share, view, comment on, edit, and sync collaborative docs. Use when someone says "view this in proof", "share to proof", "HITL this doc", or wants a shared markdown review surface for a spec, plan, or draft, for example a requirements.md from write-spec or a design.md from design-spec. Do not trigger on "proof" meaning evidence, math proofs, proof-of-concept, or "proofread this".
---

<!--
Adapted from the `ce-proof` skill in the compound-engineering plugin by
Kieran Klaassen (MIT License, see ./LICENSE). Lives in `.agents/skills/` so
Claude Code, Copilot, and Codex all read the same copy; see AGENTS.md.

Portability: everything below uses `curl` against the Proof web API and needs
no install or account. The "Local Bridge" section requires the macOS Proof
app and can be skipped on Windows/Linux. The curl examples are bash-style; in
PowerShell use double quotes or `Invoke-RestMethod`, or run them from Git Bash
or WSL2. Proof docs are token URLs on a third-party service — anyone with the
link can read and edit, so never share client data through them.
-->
...
- **Machine ID (`by` on every op, `X-Agent-Id` header):** `ai:yugastore-agent` — stable, lowercase-hyphenated, machine-parseable. Appears in marks, events, and the API response.
- **Display name (`name` on `POST /presence`):** `Yugastore Agent` — human-readable, shown in Proof's presence chips and comment-author badges.

Set the display name once per doc session by posting to presence with the `X-Agent-Id` header; Proof binds the name to that agent ID for the session. These values are the defaults for any caller of this skill; callers running HITL review (`references/hitl-review.md`) may pass a different `identity` pair if a distinct sub-agent should own the doc. Do not use `ai:yugastore` or other ad-hoc variants — identity stays uniform unless a caller explicitly overrides it.
...
- **Upstream skill handoff** — `write-spec` or `design-spec` finishes a draft and hands it off for human review before the next phase, passing the file path and title explicitly.
...
- Header: `X-Agent-Id: ai:yugastore-agent` (required for presence; include on ops for consistent attribution)
...
**Comment on text:**
```json
{"type": "comment.add", "quote": "text to comment on", "by": "ai:yugastore-agent", "text": "Your comment here", "baseToken": "<token>"}
```

**Reply to a comment:**
```json
{"type": "comment.reply", "markId": "<id>", "by": "ai:yugastore-agent", "text": "Reply text", "baseToken": "<token>"}
```

**Reply and resolve in one mutation:**
```json
{"type": "comment.reply", "markId": "<id>", "by": "ai:yugastore-agent", "text": "Fixed.", "resolve": true, "baseToken": "<token>"}
```

**Batch existing-thread comment mutations:**
```json
{"by": "ai:yugastore-agent", "baseToken": "<token>", "operations": [
...
```json
{"type": "comment.resolve", "markId": "<id>", "by": "ai:yugastore-agent", "baseToken": "<token>"}
{"type": "comment.unresolve", "markId": "<id>", "by": "ai:yugastore-agent", "baseToken": "<token>"}
```

**Suggest a replacement (pending — user must accept/reject):**
```json
{"type": "suggestion.add", "kind": "replace", "quote": "original text", "by": "ai:yugastore-agent", "content": "replacement text", "baseToken": "<token>"}
```

**Suggest and immediately apply (tracked but committed — user can reject to revert):**
```json
{"type": "suggestion.add", "kind": "replace", "quote": "original text", "by": "ai:yugastore-agent", "content": "replacement text", "status": "accepted", "baseToken": "<token>"}
```
...
```json
{"type": "suggestion.accept", "markId": "<id>", "by": "ai:yugastore-agent", "baseToken": "<token>"}
{"type": "suggestion.reject", "markId": "<id>", "by": "ai:yugastore-agent", "baseToken": "<token>"}
```
...
```json
{"type": "rewrite.apply", "content": "full new markdown", "by": "ai:yugastore-agent", "baseToken": "<token>"}
```
...
curl -X POST "https://www.proofeditor.ai/api/agent/{slug}/edit/v2" \
  -H "Content-Type: application/json" \
  -H "x-share-token: <token>" \
  -H "X-Agent-Id: ai:yugastore-agent" \
  -H "Idempotency-Key: <uuid>" \
  -d '{
    "by": "ai:yugastore-agent",
    "baseToken": "mt1:<token>",
    "operations": [
...
**Required headers:**
- `X-Agent-Id: ai:yugastore-agent` (identity for presence; keep aligned with `by`)
...
| POST | `/marks/suggest-replace` | `{"quote":"old","by":"ai:yugastore-agent","content":"new"}` |
| POST | `/marks/suggest-insert` | `{"quote":"after this","by":"ai:yugastore-agent","content":"insert"}` |
| POST | `/marks/suggest-delete` | `{"quote":"delete this","by":"ai:yugastore-agent"}` |
| POST | `/marks/comment` | `{"quote":"text","by":"ai:yugastore-agent","text":"comment"}` |
| POST | `/marks/reply` | `{"markId":"<id>","by":"ai:yugastore-agent","text":"reply"}` |
| POST | `/marks/resolve` | `{"markId":"<id>","by":"ai:yugastore-agent"}` |
| POST | `/marks/accept` | `{"markId":"<id>"}` |
| POST | `/marks/reject` | `{"markId":"<id>"}` |
| POST | `/rewrite` | Last-resort whole-doc replacement: `{"content":"full markdown","by":"ai:yugastore-agent"}` |
...
OP_RESP=$(curl -s -X POST "https://www.proofeditor.ai/api/agent/abc123/ops" \
  ...
  -d "$(jq -n --arg base "$BASE" '{type:"comment.add",quote:"text",by:"ai:yugastore-agent",text:"comment",baseToken:$base}')")
...
OP_RESP=$(curl -s -X POST "https://www.proofeditor.ai/api/agent/abc123/ops" \
  ...
  -H "X-Agent-Id: ai:yugastore-agent" \
  -H "Idempotency-Key: $(uuidgen)" \
  -d "$(jq -n --arg base "$BASE" '{type:"suggestion.add",kind:"replace",quote:"old",by:"ai:yugastore-agent",content:"new",baseToken:$base}')")
NEXT_BASE=$(printf '%s' "$OP_RESP" | jq -r '.mutationBase.token // empty')
[ -n "$NEXT_BASE" ] && BASE="$NEXT_BASE"
...
OP_RESP=$(curl -s -X POST "https://www.proofeditor.ai/api/agent/abc123/ops" \
  ...
  -H "X-Agent-Id: ai:yugastore-agent" \
  -H "Idempotency-Key: $(uuidgen)" \
  -d "$(jq -n --arg base "$BASE" '{type:"suggestion.add",kind:"replace",quote:"old",by:"ai:yugastore-agent",content:"new",status:"accepted",baseToken:$base}')")
NEXT_BASE=$(printf '%s' "$OP_RESP" | jq -r '.mutationBase.token // empty')
[ -n "$NEXT_BASE" ] && BASE="$NEXT_BASE"
...
curl -X POST "https://www.proofeditor.ai/api/agent/abc123/edit/v2?return=minimal" \
  ...
  -H "X-Agent-Id: ai:yugastore-agent" \
  -H "Idempotency-Key: $(uuidgen)" \
  -d "$(jq -n --arg base "$EDIT_BASE" '{by:"ai:yugastore-agent",baseToken:$base,operations:[{op:"find_replace_in_doc",find:"old",replace:"new",occurrence:"all"}]}')"
```
...
curl -s -X POST "https://www.proofeditor.ai/api/agent/$SLUG/presence" \
  ...
  -H "x-share-token: $TOKEN" \
  -H "X-Agent-Id: ai:yugastore-agent" \
  -d '{"name":"Yugastore Agent","status":"reading","summary":"Uploaded doc"}'
...
OP_RESP=$(curl -s -X POST "https://www.proofeditor.ai/api/agent/$SLUG/ops" \
  ...
  -H "X-Agent-Id: ai:yugastore-agent" \
  -H "Idempotency-Key: $(uuidgen)" \
  -d "$(jq -n --arg base "$BASE" '{type:"comment.add",quote:"Content here",by:"ai:yugastore-agent",text:"Added a note",baseToken:$base}')")
NEXT_BASE=$(printf '%s' "$OP_RESP" | jq -r '.mutationBase.token // empty')
[ -n "$NEXT_BASE" ] && BASE="$NEXT_BASE"
...
curl -X POST "https://www.proofeditor.ai/api/agent/$SLUG/edit/v2?return=minimal" \
  ...
  -H "X-Agent-Id: ai:yugastore-agent" \
  -H "Idempotency-Key: $(uuidgen)" \
  -d "$(jq -n --arg base "$EDIT_BASE" '{by:"ai:yugastore-agent",baseToken:$base,operations:[{op:"find_replace_in_doc",find:"Content",replace:"Updated content",occurrence:"all"}]}')"
```
