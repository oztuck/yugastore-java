...
- [ ] 6. `PriceRuleApplier` with largest-discount selection and half-up rounding; `originalPrice` and `discountPercent` transient fields on `ProductMetadata` and `ProductRanking`; apply in all three `ProductCatalogController` endpoints; unit tests for ASIN match, category match, overlap, and no match (WHEN ASIN matches, WHEN belongs to a category, WHEN more than one, WHEN an order is placed)
