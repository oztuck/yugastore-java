...
Status: proposed
Date: 2026-09-15, revised 2026-09-16 after the design interview
...
2. `products-microservice` is the only consumer for percent-off rules. It
   fetches rules from the service and returns the discounted price as `price`
   with `originalPrice` alongside, so `checkout-microservice` totals orders
   correctly without knowing rules exist. Quantity-based rules will need
   checkout to consume rules too; that is the follow-up spec's decision.
3. Products degrades to original prices when the rules service is
   unavailable, and checkout follows. Rules are a promotion, never a
   dependency for selling.
...
routes, and the local run order all change. Products now makes a network call
on a path that was local, so it needs a circuit breaker with a fallback, and
the test plan needs a fault-injection test for the rules service being down.
